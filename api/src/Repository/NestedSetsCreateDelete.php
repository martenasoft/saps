...
class NestedSetsCreateDelete extends NestedSetsBaseAbstract implements NestedSetsCreateDeleteInterface
{
    ...
    {
        if ($isSafeDelete) {
            $sql = "UPDATE `{$this->getTableName()}` SET `status` = ".(DefaultStatusInterface::STATUS_DELETED)." WHERE `id` = {$node->getId()}";
        } else {
